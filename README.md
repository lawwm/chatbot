...
**Request flow:** The client sends an HTTP request → FastAPI routes it → a service function queries MongoDB or calls the Claude API → a Jinja2 template is rendered server-side and returned as HTML. HTMX handles partial page updates by swapping targeted HTML fragments, keeping the frontend thin with no build step or client-side state management.

**Why Python:** Python was chosen primarily for its first-class integration with the Anthropic SDK, and because its dynamic typing allowed rapid iteration when defining tool schemas and the agent dispatch table — both of which involve flexible, heterogeneous data structures that benefit from duck typing over rigid interfaces.
...
**MongoDB:** Chosen for two reasons. First, its schemaless nature enables fast iteration — adding a new field to a bot or a new tool result shape requires no migration. Second, the original intent was to let the meta-agent define bot settings dynamically at runtime, including fields beyond the hardcoded set, which a document store handles naturally. That feature was ultimately cut due to time constraints, but the flexible schema still paid off during development. The tradeoff was denormalized data: deleting a bot requires manually cleaning up across `kb_content`, `kb_vectors`, `conversations`, `mistakes`, `mistakes_archive`, `roles`, and `user_roles` — something a relational DB with foreign key cascades would handle automatically.

**Web scraping:** The scraper exposes configurable settings (crawl depth, max articles, request delay, strategy) to give managers control over scope. In practice, the implementation is intentionally conservative — both because web scraping is inherently brittle against site structure changes, and because smaller, more targeted knowledge bases reduce Claude API token costs during development. More time was allocated to the agent loop than to making the scraper production-grade.

**Vector indexing:** Vector search was a requirement rather than an optimisation choice. The requirement that managers can input large documents means the full text cannot simply be pasted into the API context — both token limits and cost make that unworkable at scale. Instead, each scraped article is chunked and embedded using Voyage AI's `voyage-3-lite` model, producing a 512-dimensional dense vector per chunk. These are stored in the `kb_vectors` collection in MongoDB Atlas alongside the source text and `bot_id`. A MongoDB Atlas Vector Search index (`vector_index`) runs cosine similarity over the `embedding` field. At query time, the customer's message is embedded with the same model and a `$vectorSearch` aggregation retrieves the top-10 most semantically similar chunks filtered by `bot_id`. Retrieval is meaning-based, not keyword-based — a question like *"why was I charged twice?"* surfaces articles about duplicate transactions even if those exact words never appear in the knowledge base. If vector search returns no results, the system falls back to injecting the full scraped text into the context.

**If given more time:** The most impactful addition would have been a confirmation step before the Metabot executes any state-changing tool call — surfacing a summary of what function is about to be called and with what parameters, and requiring explicit approval before proceeding. This would address the core reliability problem with agent interfaces: the user currently has no opportunity to catch a misunderstood instruction before it modifies live data.
